...
| 提供商 | 推荐模型 | .env 配置 | 成本 |
|---|---|---|---|
| **DeepSeek** | deepseek-chat（默认） | `DEEPSEEK_API_KEY=sk-xxx` | ~¥0.1/天 |
| **DeepSeek** | deepseek-reasoner | 加 `LLM_MODEL=deepseek-reasoner` | 贵 3 倍但更准 |
| **OpenAI** | gpt-4o-mini | `OPENAI_API_KEY=sk-xxx` + `LLM_MODEL=gpt-4o-mini` | ~$0.3/天 |
| **OpenAI** | gpt-5.5 | `OPENAI_API_KEY=sk-xxx` + `LLM_MODEL=gpt-5.5` | ~$1/天 |
| **本地/其他** | 任意 | `OPENAI_API_KEY=xxx` + `LLM_BASE_URL=http://...` + `LLM_MODEL=xxx` | 免费 |

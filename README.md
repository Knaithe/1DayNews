...
- **单服务部署**：web 仪表盘 + 后台调度合为一个 systemd 服务，无需 timer
...
```bash
python src/web.py                    # http://127.0.0.1:8001（含后台 fetch/enrich 调度）
python src/web.py --no-scheduler     # 纯 web，不跑后台采集
ssh -L 8001:127.0.0.1:8001 user@srv  # 远程 SSH 隧道访问
```

Pluto Security 风格暖色卡片布局，实时搜索，药丸式源/原因/时间筛选，严重性颜色编码。默认只显示精选（pushed），可切换全量。安全加固（CSP/X-Frame-Options/nosniff），只读 SQLite + waitress，只绑 127.0.0.1。部署后只需一个 `vuln-monitor.service` 同时提供 web 和定时采集。详见 [`docs/web-dashboard.md`](docs/web-dashboard.md)。
...
部署后 web 进程内置后台调度器，每 5 分钟自动执行 `fetch → enrich`，通过 `FETCH_INTERVAL` 环境变量调整间隔（秒）。
...
部署后只有一个 systemd 服务 `vuln-monitor.service`（web + 后台调度），管理：

```bash
systemctl status vuln-monitor.service     # 状态
sudo systemctl restart vuln-monitor.service  # 重启
journalctl -u vuln-monitor.service -f     # 日志
```

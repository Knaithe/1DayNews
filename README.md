...
| 提供商 | 推荐模型 | .env 配置 | 说明 |
|---|---|---|---|
| **DeepSeek** | deepseek-v4-flash | `DEEPSEEK_API_KEY=sk-xxx` + `LLM_MODEL=deepseek-v4-flash` | 快速，便宜 |
| **OpenAI** | gpt-5.5 | `OPENAI_API_KEY=sk-xxx` + `LLM_MODEL=gpt-5.5` | 最准 |
| **OpenAI** | gpt-5.4 | `OPENAI_API_KEY=sk-xxx` + `LLM_MODEL=gpt-5.4` | 性价比 |
| **本地/中转** | 任意 | `OPENAI_API_KEY=xxx` + `LLM_BASE_URL=http://...` + `LLM_MODEL=xxx` | 自定义端点 |

高级参数（可选）：`LLM_TEMPERATURE`（默认 0.1）、`LLM_MAX_TOKENS`（默认 1024）、`LLM_TIMEOUT`（默认 60s）。自定义 system prompt 放 `/opt/vuln-monitor/llm_prompt.txt`。

不支持 temperature 或 tools 的模型会自动降级重试。

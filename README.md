...
| 通知 | `brief` | 人类可读，适合转发，自动过滤无链接/无源/噪声记录 |
| JSON | `query --json` | 程序解析 / AI 处理 |

无 CVE 的厂商公告会显示公告编号（如 `FG-IR-26-127`、`ZDI-26-298`）而非 N/A。每条记录都带原始 URL。

`brief` 有质量门槛：无 URL、无 source、reason 为 `no hit`/`excluded` 的记录自动过滤，不会进入通知视图。

从旧版 JSON cache 迁移过来的历史记录可能缺少 source/url/summary 字段，运行 `rebuild` 可一次性回填。正常 `fetch` 也会逐步回填遇到的旧记录。
